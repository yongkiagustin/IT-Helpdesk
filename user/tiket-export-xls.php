...
$sqlshow = mysqli_query($koneksi, "SELECT id_tiket, tanggal, pc_no, fullname as name, email, problem, penanganan, status FROM tiket JOIN user ON (tiket.user_id = user.user_id) WHERE user.departement_id = " . $_SESSION['departement_id']);
...
<h3>Data Tiket Helpdesk IT Departemen <?= $_SESSION['departement'] ?></h3>
